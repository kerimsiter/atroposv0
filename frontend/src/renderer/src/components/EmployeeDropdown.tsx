  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 text-left shadow-sm hover:border-[var(--color-primary)]/50"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-full grid place-items-center bg-[var(--color-primary)] text-[var(--color-primary-contrast)]">
            {(selected?.name ?? '?').charAt(0)}
          </div>
          <div className="flex-1">
            <div className="font-medium">{selected?.name || placeholder}</div>
            <div className="text-xs text-muted">{selected ? 'Selected' : 'Choose an employee'}</div>
          </div>
        </div>
      ...
      {open && (
        <div
          ref={listRef}
          className="absolute z-20 mt-2 w-full overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-xl"
          role="listbox"
        >
          <div className="p-2">
            <input
              ...
              placeholder="Search…"
              className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm outline-none focus:border-[var(--color-primary)]"
            />
          </div>
